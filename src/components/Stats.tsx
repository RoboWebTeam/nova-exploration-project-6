...
const stats = [
  ...
  { value: 30, suffix: " дней", label: "Средний срок" },
]
...
function StatItem({ value, suffix, label, isVisible }: { value: number; suffix: string; label: string; isVisible: boolean }) {
  ...
  return (
    <div className="text-center border-b border-border pb-6 md:border-b-0 md:pb-0 md:border-r md:last:border-r-0 last:border-b-0 px-2 sm:px-4 md:px-6">
      <p className="text-3xl sm:text-4xl lg:text-6xl font-medium tracking-tight mb-2">
        {count}<span className="text-orange-400">{suffix}</span>
      </p>
      <p className="text-muted-foreground text-xs sm:text-sm uppercase tracking-[0.1em] sm:tracking-[0.2em]">{label}</p>
    </div>
  )
}
